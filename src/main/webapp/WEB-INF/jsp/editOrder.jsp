...
<table class="table table-condensed">
    ...
    <c:forEach var="productInOrder" items="${listOrderDetails2}">
        <tr>
            ...
            <td><input id="count${productInOrder.getProduct().getId()}"
                       name="count${productInOrder.getProduct().getId()}}" type="number" style="width: 3em;"
                       value="${productInOrder.getQuantity()}" min="1" max="99"
                       onchange="sumPriceOrder('${productInOrder.getProduct().getId()}',${productInOrder.getProduct().getPrice()},'${IdOrder}')">
            </td>
            <td>
                <button id="${productInOrder.getProduct().getId()}"
                        onclick="deleteProductFromOrder(${productInOrder.getProduct().getId()})"
